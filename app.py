...
# 세션 레벨 알람 캐시 (포트별 중복 계산 방지 → 모바일 성능 개선)
_real_alarm_cache: dict = {}

def get_real_alarm(port, alarm_map):
    global _real_alarm_cache
    port = str(port).strip()
    if port in _real_alarm_cache:
        return _real_alarm_cache[port]
    ...
    if result is None:
        ...
        result = best(alarms) if alarms else None

    _real_alarm_cache[port] = result
    return result
...
def render_oos_table(df, col='알람등급', days_col=None, days_threshold=15,
                     date_col=None, highlight_oos=True, max_rows=300):
    """HTML 테이블 렌더러.
    highlight_oos=True 이면 알람등급=='OOS' 행 빨강.
    days_col 지정 시 해당 컬럼 값 >= days_threshold 이면 빨강.
    date_col 지정 시 날짜 컬럼으로 경과일 계산해 >= days_threshold 이면 빨강."""
    if df is None or df.empty:
        st.info("데이터 없음")
        return
    df = df.copy().reset_index(drop=True)
    if len(df) > max_rows:
        st.caption(f"⚠️ 상위 {max_rows}행 표시 (전체 {len(df)}건)")
        df = df.head(max_rows)
    ...
    for _, row in df.iterrows():
        is_oos = highlight_oos and col in df.columns and str(row.get(col, '')).strip() == 'OOS'
        is_overdue = False
        if days_col and days_col in df.columns:
            try:
                dv = pd.to_numeric(row.get(days_col), errors='coerce')
                is_overdue = pd.notna(dv) and dv >= days_threshold
            except Exception:
                pass
        if not is_overdue and date_col and date_col in df.columns:
            try:
                dt = pd.to_datetime(row.get(date_col), errors='coerce')
                if pd.notna(dt):
                    is_overdue = (_today_ts - dt).days >= days_threshold
            except Exception:
                pass
        highlight = is_oos or is_overdue
        rs = 'background:rgba(240,62,62,.13);' if highlight else ''
        fc = 'color:#f03e3e;font-weight:600;' if highlight else ''
        cells = ''.join(
...
def _get_hist_alarm(row):
    ...
    try:
        ...
        if sv and sv not in ('nan', '', 'None', 'Service Unavailable'):
            ...
            _hist_updated = True
            return sv
        # ⑤ 실시간 알람맵 조회 (현재 RAW 데이터 기준 — 아직 미복구 중인 포트에 유효)
        real = get_real_alarm(port, alarm_map)
        if real:
            _alarm_hist.setdefault('_ports', {})[port] = real
            _alarm_hist.setdefault(dk, {})[port] = real
            _hist_updated = True
            return real
    except Exception:
        pass
...
with tab2:
    ...
    render_oos_table(df_rel, highlight_oos=False, days_col='경과일', days_threshold=15)

    st.markdown('<div class="st">gREMS 알람</div>', unsafe_allow_html=True)
    _gr_date_col = next((c for c in ['발생일시','발생일','알람발생일','날짜','장애일'] if c in df_gr.columns), None)
    render_oos_table(df_gr, highlight_oos=False, date_col=_gr_date_col, days_threshold=15)
